...
Brewery-Sim is an early prototype plan for a game-like brewery simulation inspired by the real Operon brewery operating system. The current decision is to validate the gameplay loop first as a React, Vite, and TypeScript web/mobile prototype rather than starting in Unity or integrating with Operon immediately.
...
This is the right choice for the current stage because the main unknown is not rendering technology; it is whether the brewery gameplay loop is fun. A web prototype lets the team test the loop quickly, keep the simulation logic readable, and avoid investing in a full game-engine workflow before the core mechanics are proven.

The first prototype should be a local-only, single-player garage brewery simulation with a game-like scene UI. It should not feel like a production dashboard, ERP, brewery MES, or Operon admin screen.
...
Clicking equipment opens a contextual action panel with only relevant choices. Examples:
