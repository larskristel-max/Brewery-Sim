...
func _run() -> void:
	...
	_expect(_fits_in_viewport(instance, instance.ui.title_card), "Opening title overflowed the phone viewport")
	var opening := _find_button(instance.ui.title_screen, "Begin the story")
	_expect(opening != null, "Phone opening title did not expose the story")
