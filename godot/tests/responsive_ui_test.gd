...
func _run() -> void:
	...
	_expect(instance.ui.rotation_gate.visible, "Portrait gate did not protect the opening cinematic")
	...
	_expect(instance.ui.rotation_gate.visible, "Blocked portrait input dismissed the rotation gate")

	root.size = Vector2i(844, 390)
	await _settle()
	instance._apply_responsive_layout()
	await _settle()
	_expect(not instance.portrait_layout, "Phone rotation did not restore the landscape opening")
	_expect(instance.mobile_landscape_layout, "Landscape phone did not activate the compact opening layout")
	_expect(root.content_scale_size == Vector2i(960, 540), "Phone landscape did not select the compact landscape content scale")
	_expect(not instance.ui.rotation_gate.visible, "Landscape phone kept the pre-cinematic rotation gate visible")
	if opening: opening.pressed.emit()
	await _settle()
	var advance := _find_named_button(instance.ui.prologue, "AdvancePrologue")
	_expect(advance != null, "Landscape prologue did not expose a tap-to-advance layer")
	_expect(_fits_in_viewport(instance, instance.ui.prologue.subtitle_panel), "Landscape prologue subtitles overflowed the viewport")
	var prologue_index := int(instance.ui.prologue.shot_index)
	instance.ui.prologue.shot_elapsed = 999.0
	await _settle()
	_expect(int(instance.ui.prologue.shot_index) == prologue_index, "Landscape prologue advanced without a tap")
	var skip := _find_button(instance.ui.prologue, "SKIP")
	if skip: skip.pressed.emit()
	await _settle()
	_expect(_fits_in_viewport(instance, instance.ui.customization_card), "Appointment form overflowed the landscape phone viewport")
	_expect(instance.ui.customization.find_children("*", "OptionButton", true, false).is_empty(), "Landscape appointment retained the coat selector")
	...
	_expect(not instance.ui.rotation_gate.visible, "Landscape gate covered the awakening cinematic")
	...
	_expect(_fits_in_viewport(instance, instance.ui.awakening.subtitle_panel), "Landscape awakening subtitles overflowed the viewport")
	var door_button := _find_named_button(instance.ui.awakening, "OpenStableDoors")
	_expect(door_button != null and _fits_in_viewport(instance, door_button), "Landscape stable-door interaction was not usable")
	var awakening_index := int(instance.ui.awakening.shot_index)
	instance.ui.awakening.shot_elapsed = 999.0
	await _settle()
	_expect(int(instance.ui.awakening.shot_index) == awakening_index, "Landscape awakening cinematic advanced without a tap")
	...
	await _settle()
	_expect(not instance.ui.rotation_gate.visible, "Landscape phone showed the rotation gate when gameplay began")
	...
	_expect(instance.ui.command_dock.size.y <= 152.0, "Expanded landscape context obscured too much of the brewery (%.0f px)" % instance.ui.command_dock.size.y)
	...
	_expect(_find_button(instance.ui.actions, "Recommission the copper") != null, "Landscape context did not expose the selected equipment action")
