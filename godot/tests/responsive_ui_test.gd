...
func _run() -> void:
	...
	_expect(instance.ui.rotation_gate.visible, "Portrait phone did not ask the player to rotate")
	...
	_expect(_fits_in_viewport(instance, instance.ui.title_card), "Opening title overflowed the landscape phone viewport")
	var opening := _find_button(instance.ui.title_screen, "Begin the story")
	_expect(opening != null, "Landscape phone opening title did not expose the story")
	...
	await _settle()
	_expect(instance.ui.customization_story.visible, "Landscape appointment lost its story context")
	...
	_expect(begin != null, "Landscape appointment form did not expose the stable key")
