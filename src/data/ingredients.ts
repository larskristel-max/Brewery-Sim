...
export const getIngredient = (ingredientId: IngredientId): Ingredient => {
  const ingredient = ingredients.find((item) => item.id === ingredientId);
  if (!ingredient) throw new Error(`Unknown ingredient: ${ingredientId}`);
  return ingredient;
};

export const createIngredientStock = (): Record<IngredientId, IngredientStock> =>
  Object.fromEntries(
    ingredients.map((ingredient) => [
      ingredient.id,
      {
        amount:
          ingredient.id === 'pilsner-malt'
            ? 10
