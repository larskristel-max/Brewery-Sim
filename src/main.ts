...
const equipmentIcon = (equipmentId: EquipmentId) => ({ kettle: '♨️', fermenter: '🛢️', bottler: '📦' })[equipmentId];
...
const isNextTapTarget = (target: EquipmentId | 'cases') => currentWorkflowStage(state).tapTarget === target;
...
root.addEventListener('click', (event) => {
  ...
  if (action === 'use-equipment') dispatch({ type: 'use-equipment', equipmentId: target.dataset.equipmentId as EquipmentId });
