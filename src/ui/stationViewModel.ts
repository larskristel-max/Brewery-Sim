...
export const equipmentCapacityLabel = (equipment: Equipment): string => {
...
export const createStationViewModel = (context: StationViewModelContext) => {
  const { state } = context;
  ...
  const equipmentSceneStatus = (equipmentId: EquipmentId): EquipmentStatus => {
  ...
  const equipmentInstanceStatus = (equipment: GarageSceneEquipmentInstance): EquipmentStatus => {
  ...
  const stationPanelContext = (): StationPanelContext => ({
    state,
