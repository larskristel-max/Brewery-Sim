...
import { caseCountLabel, caseDefinitionExplanation, formatClock, formatCurrency, formatGameDate, garageSpaceAvailable, ingredientAmountLabel, orderCost, recipeMissingIngredients, recipeMissingOrderSummary, recipeRequirementSummary, recipeSupplyBreakdown, storageCapacityByArea, storageOverflowByArea, storageUseByArea, visibleRecipes } from '../game/selectors.js';
...
const garageSpaceUsed = (state) => state.garageSpaceUsed;
const garageSpaceLimit = (state) => state.garageSpaceLimit;
...
const renderVerdictLine = (verdict) => `<small><strong>${verdict.qualityBand.toUpperCase()}</strong> - ${verdict.headline} Advice: ${verdict.sellAdvice}. Stability ${verdict.stabilityRisk}/100.</small><small>${verdict.sensoryNotes.slice(0, 2).join(' ')}</small><small>${verdict.likelyCauses.slice(0, 2).join(' ')}</small>`;
const renderLotRecoveryActions = (lot) => {
    const competitionButton = lot.verdict.sellAdvice !== 'recall' && lot.verdict.sellAdvice !== 'dump' && lot.verdict.qualityBand !== 'unsafe'
        ? `<button data-action="competition-entry" data-lot-id="${lot.id}" type="button">Enter judging</button>`
        : '';
    if (lot.verdict.sellAdvice === 'sell' && lot.verdict.stabilityRisk < 30)
        return competitionButton ? `<div class="hotspot-actions">${competitionButton}</div>` : '';
    const discountButton = lot.verdict.sellAdvice === 'discount' || lot.verdict.qualityBand === 'flawed'
        ...
        : '';
    const recallButton = lot.verdict.sellAdvice === 'recall' || lot.verdict.qualityBand === 'unsafe' || lot.verdict.stabilityRisk >= 70
...
const renderPackagingButtons = (batch) => `
...
`;
const renderBatchBoard = ({ state, batchRemainingLabel, stepLabel }) => {
    ...
    const lotCards = state.finishedBeerLots.length > 0
        ...
        : '';
    const historyCards = state.breweryHistory.length > 0
...
const renderStorageStatus = (state) => {
...
`;
const equipmentStoreButtonState = (state, item) => {
    ...
    if (projectedSpace > garageSpaceLimit(state))
        return { disabled: true, label: 'No space', reason: `${projectedSpace}/${garageSpaceLimit(state)} garage space`, className: 'upgrade-locked' };
...
const renderEquipmentStoreCard = (state, item) => {
...
const ingredientCartButtonState = (state, ingredient) => {
    const incoming = incomingForIngredient(state, ingredient.id);
    if (state.cash < ingredient.packPrice)
...
const renderIngredientCartCard = (state, ingredient) => {
    const stock = state.inventory.ingredients[ingredient.id];
...
const renderCampaignSupplyHint = (state) => {
...
const renderCampaignEquipmentHint = (state) => {
    if (state.campaign.missionId !== 'bucket-empire')
        return '';
    return `
...
const renderIngredientCart = (state) => `
...
`;
const renderShopSectionCards = (state) => {
...
const renderEquipmentStore = ({ state, selectedShopSection }) => `
...
`;
const overlayContent = (context) => {
    if (context.activeOverlay === 'production')
        return renderBatchBoard(context);
    if (context.activeOverlay === 'inventory')
        return renderInventory(context.state);
    if (context.activeOverlay === 'upgrades')
        return renderEquipmentStore(context);
    if (context.activeOverlay === 'log')
        return renderEventLog(context.state);
    return '';
};
const overlayTitle = (activeOverlay) => ({ production: 'Production', inventory: 'Inventory detail', upgrades: 'Shop cart', log: 'Clipboard log' })[activeOverlay ?? 'production'];
export const renderFocusOverlay = (context) => context.activeOverlay
    ? `
