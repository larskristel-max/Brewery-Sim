...
import { caseCountLabel, caseDefinitionExplanation, cleaningPlanForEquipment, formatClock, formatCurrency, formatGameDate, recipeCanStart, recipeCategories, recipeIngredientCost, saleConsequencePreview, saleCasesForChannel, salesChannels, saleValueForChannel, visibleRecipes } from '../game/selectors.js';
...
const renderSaleConsequencePreview = (state, channelId, cases) => {
    if (cases <= 0)
        return 'No sellable cases';
...
const renderPackagingChoiceButtons = (batch) => `
...
`;
const renderTransferChoiceButtons = (batch) => `
...
`;
const conditioningSummary = (batch) => `Carbonation ${batch.conditioningState.carbonationProgress}% - CO2 ${batch.conditioningState.co2Integration} - pressure ${batch.conditioningState.packagePressureRisk} - refermentation ${batch.conditioningState.refermentationRisk}`;
const renderPromiseNote = (state) => state.demand.deadlineDay
    ...
    : '';
const renderRecoveryActions = (state) => {
...
const renderCompetitionActions = (state) => {
...
const renderLeadLotStory = (state) => {
