...
import { formatGameDate, ownedByStation, recipeMissingIngredients, recipeMissingOrderSummary, recipeRequirementSummary } from './selectors.js';
...
export const campaignMissions = {
    'barbecue-text': {
        ...
        characterId: 'samira',
        act: 'Garage Doors Up',
        message: 'Big barbecue in six weeks. Could you bring 4 cases of Garage Blonde? People keep asking about that garage beer. I said yes before asking you. Sorry. Sort of.',
        phoneThread: [
    ...
    'empty-shelf': {
        ...
        act: 'Back Already?',
        message: 'The barbecue beer vanished. That is good news. Your shelf, however, now has the confident emptiness of someone who forgot beer needs ingredients.',
        phoneThread: [
    ...
    'bucket-empire': {
        ...
        message: 'You have one bucket doing honest work and another beer promise arriving behind it. That is not production planning. That is hoping plastic can multitask.',
        phoneThread: [
    ...
    'uncle-nico-wedding': {
        ...
        message: 'I told the wedding people you make beer now. They loved this. I may have said 10 cases. Four months is enough, right?',
        phoneThread: [
    ...
    'warm-garage-week': {
        ...
        message: 'The garage smells like bread, flowers, and ambition. Nice for humans. Slightly dramatic for yeast.',
        phoneThread: [
    ...
    'sticky-bucket': {
        ...
        characterId: 'bucket',
        act: 'Clean Is A Verb',
        message: 'The bucket no longer smells neutral. It smells like old cereal and optimism. That is not a fermentation profile.',
        phoneThread: [
    ...
    'labels-at-midnight': {
        ...
        characterId: 'samira',
        act: 'Looks Like Beer',
        message: 'A private event wants bottles that look less like they were smuggled out from under a workbench. Rustic is good. Random is less good.',
        phoneThread: [
    ...
    'first-festival': {
        ...
        act: 'Public Pour',
        message: 'A small summer beer table has one open spot. Eight cases, presentable bottles, no weird package surprises. If people talk, bars listen.',
        phoneThread: [
    ...
    'first-bar-account': {
        ...
        message: 'Bring me a few bottles. If they are good, we can talk about a small order. If they are weird, we can also talk, but it will be shorter.',
        phoneThread: [
    ...
    'household-summit': {
        ...
        act: 'Garage Treaty',
        message: 'There are bottles in the laundry basket and something is bubbling next to the freezer. We need a garage treaty.',
        phoneThread: [
    ...
    'sandbox-unlocked': {
        ...
        act: 'Sandbox',
        message: 'The garage works. Not elegantly, not quietly, and not without witnesses. But it works.',
        phoneThread: [
            'The garage works.',
...
const missionIndex = (missionId) => campaignMissionOrder.indexOf(missionId);
export const isCampaignComplete = (state) => state.campaign.missionId === 'sandbox-unlocked';
export const isMissionSeen = (state, missionId = state.campaign.missionId) => state.campaign.seenMissionIds.includes(missionId);
export const campaignHasReached = (state, missionId) => missionIndex(state.campaign.missionId) >= missionIndex(missionId) || state.campaign.completedMissionIds.includes(missionId);
export const campaignAllowsAdvancedRecipes = (state) => campaignHasReached(state, 'first-bar-account');
export const campaignAllowsTemperature = (state) => campaignHasReached(state, 'warm-garage-week');
export const campaignAllowsCleaning = (state) => campaignHasReached(state, 'sticky-bucket');
export const campaignAllowsFormalBuyers = (state) => campaignHasReached(state, 'first-bar-account');
export const campaignAllowsPressure = (state) => campaignHasReached(state, 'household-summit');
export const campaignVisibleSalesChannels = (state) => {
    if (campaignAllowsFormalBuyers(state))
        return ['friends-family', 'private-event', 'local-bar'];
    if (campaignHasReached(state, 'uncle-nico-wedding'))
        return ['friends-family', 'private-event'];
    return ['friends-family'];
};
export const campaignHiddenSystems = (state) => ({
...
const missingGarageBlondeSupplies = (state) => recipeMissingIngredients(state, garageBlonde());
const hasIncomingGarageBlondeSupplies = (state) => {
    const missing = missingGarageBlondeSupplies(state);
    if (missing.length === 0)
        return false;
    return missing.every((item) => {
        const incoming = state.pendingOrders.reduce((total, order) => total + order.items.filter((orderItem) => orderItem.ingredientId === item.ingredientId).reduce((sum, orderItem) => sum + orderItem.amount, 0), 0);
        return incoming >= item.amount;
    });
};
const firstActiveBatch = (state) => state.batches[0];
const firstLoopNextStep = (state) => {
    const batch = state.batches.find((item) => item.recipeId === 'garage-blonde');
    if (state.demand.casesSold >= 4)
        return 'Order complete. Tap the story card to read the barbecue feedback.';
    if (state.inventory.cases > 0)
        return "Tap the pallet to deliver Samira's barbecue cases.";
    if (!batch)
        return 'Tap the stock pot to brew Garage Blonde.';
    if (batch.step === 'brewing')
    ...
    if (batch.step === 'fermenting')
        return 'Tap the fermenter to let fermentation finish.';
    if (batch.step === 'awaiting-packaging')
        return 'Tap the fermenter to move Garage Blonde to bottling.';
    if (batch.step === 'packaging' || batch.step === 'bottle-conditioning')
        return 'Tap the bottling bench to finish packaging.';
    return 'Tap the stock pot to brew Garage Blonde.';
};
const firstLoopTarget = (state) => {
    const batch = state.batches.find((item) => item.recipeId === 'garage-blonde');
    if (state.inventory.cases > 0)
        return 'cases';
    if (!batch || batch.step === 'brewing' || batch.step === 'awaiting-transfer')
        return 'kettle';
    if (batch.step === 'fermenting' || batch.step === 'awaiting-packaging')
        return 'fermenter';
    return 'bottler';
};
export const campaignNextStep = (state) => campaignView(state).nextStep;
export const campaignPrimaryTarget = (state) => campaignView(state).primaryTarget;
export const campaignView = (state) => {
    ...
    let progressLabel = '';
    let nextStep = '';
    let primaryTarget = 'kettle';
    if (mission.id === 'barbecue-text') {
        progress = Math.min(100, Math.round((Math.min(state.demand.casesSold, 4) / 4) * 100));
        if (progress === 0 && activeBatch)
            progress = 30;
        if (state.inventory.cases > 0 && state.demand.casesSold < 4)
            progress = 70;
        ...
        if (state.batches.some((batch) => batch.recipeId === 'garage-blonde'))
            progress = 100;
...
export const markCampaignMissionSeen = (state, missionId = state.campaign.missionId) => {
    if (!state.campaign.seenMissionIds.includes(missionId)) {
        state.campaign.seenMissionIds = [...state.campaign.seenMissionIds, missionId];
    }
};
export const completeCampaignMission = (state, missionId = state.campaign.missionId) => {
    ...
    if (nextMission)
        state.campaign.missionId = nextMission;
};
export const syncCampaignAfterAction = (state, actionType) => {
