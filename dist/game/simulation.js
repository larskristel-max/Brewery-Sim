...
const advanceBatch = (state, batch, seconds) => {
    ...
    while (batch.step !== 'ready' && batch.stepProgress >= 60) {
        ...
        if (completedStep === 'mashing')
            addEvent(state, `${batch.recipeName} moved into fermentation.`);
        if (completedStep === 'fermenting')
            addEvent(state, `${batch.recipeName} finished fermenting. Package it when ready.`);
        if (completedStep === 'packaging')
            addEvent(state, `${batch.recipeName} is boxed and ready to sell.`);
    }
};
