...
const randomEventForDay = (state) => {
    ...
    if (eventIndex === 0)
        return 'Local café requests extra cases for a tasting board.';
    if (eventIndex === 1) {
    ...
    if (eventIndex === 3)
        return 'Contamination warning: dirty fermenters will drag quality down fast.';
    if (eventIndex === 4) {
...
const advanceBatch = (state, batch, seconds) => {
    ...
    while (batch.step !== 'ready' && batch.stepProgress >= 100) {
        ...
        if (completedStep === 'fermenting')
            addEvent(state, `${batch.recipeName} finished fermenting. Tap the bench capper to package.`);
...
const startBatch = (next, recipeId) => {
    ...
    const canAffordIngredients = next.inventory.grain >= recipe.grainCost &&
...
export const reduceGame = (state, action) => {
    ...
    if (action.type === 'use-equipment') {
        ...
        if (action.equipmentId === 'kettle')
            return startBatch(next, 'garage-pale');
