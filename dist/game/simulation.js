...
import { activeOwnedEquipment, bottlesPerCase, caseCountLabel, cleaningPlanForEquipment, durationLabel, equipmentConditionTier, finishedBeerCaseCount, formatCurrency, garageSpaceAvailable, orderCost, recipeBatchCapacity, recipeMissingIngredients, recipeOrderItems, saleConsequencePreview, salesChannels, storageOverflowByArea, totalStorageOverflow } from './selectors.js';
...
const fallbackVerdict = (quality, recipeName) => ({
...
const cloneState = (state) => ({
    ...
    customerMemory: Object.fromEntries(Object.entries(state.customerMemory ?? {}).map(([id, memory]) => [id, { ...memory, notes: [...memory.notes] }])),
...
const addIdentityTag = (state, tag) => {
...
const addIdentityPoints = (state, path, points, reason) => {
    const previous = state.identityScores[path] ?? 0;
    state.identityScores[path] = Math.max(0, previous + points);
    if (state.identityScores[path] >= 5 && previous < 5)
        addIdentityTag(state, path.replace(/-/g, ' '));
    if (points > 0)
        addHistory(state, { kind: 'identity', title: `${path.replace(/-/g, ' ')} +${points}`, detail: reason });
};
const updateBreweryTier = (state) => {
    ...
    const nextTier = state.reputation >= 24 && state.flagshipRecipeIds.length >= 2
        ? 'regional'
...
const promiseIdForDemand = (demand) => `promise-${demand.customerId ?? demand.channelId}${demand.requestedRecipeId ? `-${demand.requestedRecipeId}` : ''}-${demand.deadlineDay ?? 'open'}-${demand.casesRequested}`;
const ensureActivePromise = (state) => {
    if (!state.demand.promiseLocked || !state.demand.customerId || !state.demand.deadlineDay || !state.demand.minimumQualityBand || !state.demand.packagingExpectation)
        return null;
...
const syncActivePromiseProgress = (state) => {
    const promise = ensureActivePromise(state);
    if (!promise)
        return;
    promise.deliveredCases = state.demand.casesSold;
    if (state.demand.missedPromise)
        promise.status = 'missed';
    else if (state.demand.casesSold >= state.demand.casesRequested)
        promise.status = 'fulfilled';
};
const hasOpenLockedPromise = (state) => Boolean(state.demand.promiseLocked && !state.demand.missedPromise && state.demand.casesSold < state.demand.casesRequested);
const promiseTemplate = (state, promiseId) => {
    const templates = {
        'mira-regular-tap': {
            demand: {
...
const chooseBreweryPromise = (state, promiseId) => {
...
const raiseSanitationDebt = (state, area, amount) => {
...
const demandForDay = (state, day) => {
    ...
    if (flagshipRecipeId && day > 1) {
        ...
        const flagshipChannelId = state.reputation >= 9 ? 'local-bar' : 'friends-family';
...
const substitutionEffect = (missingIngredientId, substituteIngredientId, amount) => {
    const missing = getIngredient(missingIngredientId);
    const substitute = getIngredient(substituteIngredientId);
    const effect = missing.category === 'hops'
...
const substitutionPlanForRecipe = (state, recipe) => {
    const available = Object.fromEntries(Object.entries(state.inventory.ingredients).map(([id, stock]) => [id, stock.amount]));
    const consumption = [];
    const substitutions = [];
...
const defaultConditioningState = () => ({
    carbonationProgress: 0,
    co2Integration: 'rough',
    refermentationRisk: 0,
    packagePressureRisk: 0
});
const fermentationReadinessFor = (state, recipe, progress, gravityChecked = false) => {
...
const applyFermentationReadiness = (state, batch) => {
...
const conditioningLabel = (state) => `${state.carbonationProgress}% carbonation, CO2 ${state.co2Integration}, pressure risk ${state.packagePressureRisk}`;
const advanceConditioningState = (batch) => {
...
const finishConditioningState = (batch) => {
...
const activeCustomerId = (state, channelId) => {
    const account = state.demand.accountName.toLowerCase();
    if (account.includes('samira') || state.campaign.missionId === 'barbecue-text')
        return 'samira';
    if (account.includes('nico') || account.includes('wedding'))
        return 'nico';
    if (account.includes('mira') || channelId === 'local-bar')
        return 'mira';
    if (channelId === 'restaurant')
        return 'restaurant';
    if (channelId === 'private-event')
        return 'festival';
    return 'samira';
};
const qualityRank = (qualityBand) => ({ unsafe: 0, bad: 1, flawed: 2, solid: 3, excellent: 4 })[qualityBand];
const packagingMeetsExpectation = (verdict, expectation) => {
    if (!expectation || expectation === 'any')
        return true;
    if (expectation === 'presentable')
        return verdict.presentationScore >= 60;
    if (expectation === 'clean-label')
        return verdict.presentationScore >= 75 && qualityRank(verdict.qualityBand) >= qualityRank('solid');
    return false;
};
const rememberCustomerNote = (state, customerId, note, trustDelta) => {
...
const identityPathForRecipe = (recipe) => {
    const style = recipe.style.toLowerCase();
    if (style.includes('pils') || style.includes('lager'))
        return 'clean-lager-specialist';
    if (style.includes('saison') || style.includes('kveik') || style.includes('wheat'))
        return 'farmhouse-saison-brewer';
    if (style.includes('ipa'))
        return 'hype-ipa-brewery';
    if (style.includes('stout'))
        return 'local-pub-workhorse';
...
const durationForStep = (state, batch, step) => {
    ...
    if (step === 'brewing')
        duration *= brewdayEffect(recipe, batch.brewdayApproach ?? 'standard').time;
    if (step === 'packaging')
        duration *= packagingModeEffect(batch.packagingMode ?? 'standard').time;
    if (step === 'fermenting')
...
const batchVerdictFor = (batch, finalQuality) => {
    const recipe = getRecipe(batch.recipeId);
    const readiness = batch.fermentationReadiness ?? fermentationReadinessFor({ fermenterTemperatureC: 18 }, recipe, 100, true);
    ...
    if (packaging.presentationScore >= 78)
        sensoryNotes.push('Bottles look intentional instead of garage-random.');
    ...
    if (batch.faultEventsTriggered.length > 0)
        likelyCauses.push('Recorded process fault during the batch.');
    const qualityBand = stabilityRisk >= 72 || adjustedQuality < 30
        ? 'unsafe'
        : adjustedQuality >= 84
            ? 'excellent'
            : adjustedQuality >= 64
    ...
    if (qualityBand === 'excellent')
        legacyTags.push('flagship candidate');
    if (qualityBand === 'solid' && batch.brewdayApproach === 'fast')
        legacyTags.push('fast but clean');
    if (style.includes('saison') || style.includes('kveik'))
        legacyTags.push('seasonal story');
    const headline = qualityBand === 'excellent'
...
const finishConditionedBatch = (state, batch) => {
    ...
    if (qualityRank(verdict.qualityBand) >= qualityRank('solid')) {
        ...
        if (verdict.stabilityRisk <= 12)
            addIdentityPoints(state, 'regional-consistency', 1, `${batch.recipeName} packaged with stable quality.`);
    }
    updateBreweryTier(state);
...
const advanceBatch = (state, batch, minutes) => {
    ...
    if (batch.step === 'fermenting')
        applyFermentationReadiness(state, batch);
    if (batch.step === 'bottle-conditioning')
        advanceConditioningState(batch);
...
const rolloverOneDay = (state) => {
    ...
    if (lockedPromiseStillOpen) {
        if (state.demand.deadlineDay && state.day > state.demand.deadlineDay && !state.demand.missedPromise) {
            ...
            if (state.demand.customerId)
                rememberCustomerNote(state, state.demand.customerId, `${state.demand.accountName} deadline was missed.`, -2);
        ...
        if (state.demand.flagshipRequest && state.demand.requestedRecipeName)
            addEvent(state, `Flagship pull: this order only counts if you deliver ${state.demand.requestedRecipeName}.`);
...
const startBatch = (next, recipeId, approach = 'standard', allowSubstitutions = false) => {
...
const transferAwaitingBatch = (next, batchId, mode = 'careful') => {
...
const packageAwaitingBatch = (next, batchId, mode = 'standard') => {
...
const checkGravity = (next, batchId) => {
...
const packageEarly = (next, batchId) => {
    const batch = next.batches.find((item) => item.step === 'fermenting' && (!batchId || item.id === batchId));
    if (!batch) {
...
const waitUntilReady = (next, batchId) => {
    ...
    const energyCost = batch.step === 'brewing'
...
const sellCases = (next, requestedCases, channelOverride) => {
    ...
    const previewSoldLots = [];
    next.finishedBeerLots.forEach((lot) => {
        if (casesToAcceptPreview <= 0)
            return;
    ...
    if (worstVerdict) {
        ...
        const reaction = worstVerdict.qualityBand === 'excellent'
...
const highestRiskLot = (state) => [...state.finishedBeerLots].sort((a, b) => (b.verdict?.stabilityRisk ?? 0) - (a.verdict?.stabilityRisk ?? 0) || qualityRank(a.verdict?.qualityBand ?? 'solid') - qualityRank(b.verdict?.qualityBand ?? 'solid'))[0];
const recoverLot = (next, actionId, lotId) => {
    ...
    if (actionId === 'replacement-gesture') {
        ...
        const activePromise = ensureActivePromise(next);
        if (activePromise && activePromise.status === 'missed')
            activePromise.status = 'replaced';
...
const enterCompetition = (next, lotId) => {
...
export const reduceGame = (state, action) => {
    ...
    if (action.type === 'use-equipment') {
        ...
        if (action.equipmentId === 'fermenter') {
            ...
            if (waiting)
                return transferAwaitingBatch(next, waiting.id, 'careful');
    ...
    if (action.type === 'transfer-batch')
        return transferAwaitingBatch(next, action.batchId, action.transferMode ?? 'careful');
    if (action.type === 'check-gravity')
        return checkGravity(next, action.batchId);
    if (action.type === 'package-early')
        return packageEarly(next, action.batchId);
    if (action.type === 'start-packaging')
        return packageAwaitingBatch(next, action.batchId, action.packagingMode ?? 'standard');
    ...
    if (action.type === 'clean-equipment') {
        ...
        if (action.equipmentId === 'kettle')
            next.sanitationDebt.brewhouse = Math.max(0, next.sanitationDebt.brewhouse - 32);
        if (action.equipmentId === 'fermenter')
            next.sanitationDebt.fermentation = Math.max(0, next.sanitationDebt.fermentation - 34);
        if (action.equipmentId === 'bottler')
            next.sanitationDebt.packaging = Math.max(0, next.sanitationDebt.packaging - 38);
        if (action.equipmentId === 'bottler' || action.equipmentId === 'fermenter')
            next.sanitationDebt.transferPath = Math.max(0, next.sanitationDebt.transferPath - 24);
